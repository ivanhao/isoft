...
$model_main->ism_supplier=$_POST["ism_supplier_name"];
...
if($_GET["ism_writer_name"]!="")$map["f.emp_name"]=array("like","%{$_GET['ism_writer_name']}%");
...
join('twms_store as c on sto_id=iss_store')->
join('twms_supplier as d on ism_supplier=sup_id')->
join('twms_employee as e on ism_operator=e.emp_id')->
join('twms_employee as f on ism_writer=f.emp_id')->
...
field('a.ism_sn,c.emp_name as ism_operator_name,d.emp_name as ism_writer_name,b.sup_name as ism_supplier_name')->find();
$model_sub=M("instore_sub");
...
where(array("ism_id"=>$_GET['ism_id']))->
field('a.*,c.emp_name as ism_operator_name,d.emp_name as ism_writer_name,b.sup_name as ism_supplier_name')->find();
$model_sub=M('instore_sub');
$list_sub=$model_sub->alias('a')->join('twms_product on iss_prod=prod_id')->
join('twms_prod_cate on iss_cate=pdca_id')->join('twms_store on sto_id=iss_store')->
where(array('iss_mainid'=>$_GET['ism_id']))->order('iss_id asc')->select();
$model_store=M('store');
$list_store=$model_store->select();
$this->assign('list_store',$list_store);
