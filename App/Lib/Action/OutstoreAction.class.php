...
$list=$model->alias("a")->join("twms_outstore_sub as b on osm_id=oss_mainid")->
join('twms_store as c on sto_id=oss_store')->
join('twms_customer as d on a.osm_customer=d.cust_id')->
join('twms_employee as e on a.osm_operator=e.emp_id')->
join('twms_employee as f on a.osm_writer=f.emp_id')->
where($map)->
field("a.*,sum(oss_price*oss_count) as total,sum(oss_count) as count,cust_name,e.emp_name as osm_operator_name,oss_prodname as osm_writer_name,oss_price price,c.sto_name")->
group("oss_mainid")->limit($Page->firstRow.','.$Page->listRows)->order("osm_id desc")->select();
$list_sum=$model->alias("a")->join("twms_outstore_sub as b on a.osm_id=b.oss_mainid")->where($map)->field("sum(oss_count) as sumNum,sum(oss_price*oss_count) as sumtotal")->select();
