...
join('twms_store on stk_store=sto_id')->order('stk_id desc')->limit($Page->firstRow.','.$Page->listRows)->select();
...
$model=D("Stock");
$list=$model->alias('a')->join('twms_prod_cate as b on stk_cate=pdca_id')->
join('twms_store on stk_store=sto_id')->order('stk_id desc')->select();
$this->assign("list",$list);
$this->display();
}
